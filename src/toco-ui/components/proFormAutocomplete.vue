...
type FormAutocompleteProps = PropFormFieldProps &
  Omit<ProFieldPropsType<'autocomplete'>, 'valueType'>;
const props = withDefaults(
  defineProps<FormAutocompleteProps>(),
  getProFormFieldInitProps()
) as FormAutocompleteProps;

const ProFormFieldProps: Ref<PropFormFieldProps> = computed(() => {
  return getProFormFieldProps<'autocomplete'>(props);
});

const ProFieldProps: Ref<ProFieldPropsType> = computed(() => {
  return getProFieldProps(props, 'autocomplete');
});

...
type FormColorPickerProps = PropFormFieldProps &
  Omit<ProFieldPropsType<'colorPicker'>, 'valueType'>;
const props = withDefaults(
  defineProps<FormColorPickerProps>(),
  getProFormFieldInitProps()
) as FormColorPickerProps;

const ProFormFieldProps: Ref<PropFormFieldProps> = computed(() => {
  return getProFormFieldProps<'colorPicker'>(props);
});

const ProFieldProps: Ref<ProFieldPropsType> = computed(() => {
  return getProFieldProps(props, 'colorPicker');
});

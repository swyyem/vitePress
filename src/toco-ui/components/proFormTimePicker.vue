...
export type FormTimePickerProps = PropFormFieldProps &
  Omit<ProFieldPropsType<'timePicker'>, 'valueType'>;
const props = withDefaults(
  defineProps<FormTimePickerProps>(),
  getProFormFieldInitProps()
) as FormTimePickerProps;

const ProFormFieldProps: Ref<PropFormFieldProps> = computed(() => {
  return getProFormFieldProps<'timePicker'>(props);
});

const ProFieldProps: Ref<ProFieldPropsType> = computed(() => {
  return getProFieldProps(props, 'timePicker');
});

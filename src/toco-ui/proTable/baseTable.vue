...
defineSlots<{
  // 操作列插槽
  'column-operating': (props: { actions: ProTableProviderProps['actions'] }) => any;
  // 展开行插槽
...
const rowSelectionProps = computed(() => {
  ...
  const { type, selectedRowKeys, selectable = (_data: AnyObject) => true, ...rest } = rowSelection;
...
const getSelectionRows = () => {
  // 输出 rowData[]
  const rows = getDataFromKeys(selectionRowKeys.value, tableData.value, getRowKey.value);
  return rows;
};
...
watch(
  ...
  newVal => {
    ...
    if (formRef.value) {
      ...
      if (modifys.length > 0) {
        formRef.value.setFormValues(getValueFromTable(modifys, getRowKey.value));
      }
      if (removes.length > 0) {
        formRef.value.removeFormValues(getValueFromTable(removes, getRowKey.value));
      }
    }
...
const { navigateRow, setNavigateRow, scrollToCurrentRow, setRowClassName, setHoverIndex } =
  useNavigate({
    deferReadyExecutor,
    tableContentRef,
    rowSelectionProps,
...
const getRecordSet = (): ProRecordDataType<T> => {
  ...
  // 删除的数据
  return {
    add: getListFromForm<T>(addSet as T[], formRef.value?.getFormFullValues(), getRowKey.value),
  ...
  >
    <ProTableToolbar v-if="showToolbar" v-bind="toolbar">
      <template #toolbar-filters v-if="$slots['toolbar-filters']">
        <slot name="toolbar-filters" :onSearch="handleSearch" :searchForm="searchForm"></slot>
      </template>
      <template #toolbar-search v-if="$slots['toolbar-search']">
        <slot name="toolbar-search" :onSearch="handleSearch" :searchForm="searchForm"></slot>
      </template>
    ...
    >
      <div class="pro-table--body__main" :class="props.bodyMainClassName">
        ...
        <div v-if="props.recordCreatorProps !== false && editable" class="pro-table--operate">

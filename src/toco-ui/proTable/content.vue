...
import { computed, h, inject, useAttrs, ref, watch, useSlots, shallowRef, provide } from 'vue';
...
defineSlots<{
  // 展开行插槽
  expand: (props: { row: AliasAny; column: AliasAny; $index: number }) => AliasAny;
  // 空状态插槽
  empty: () => AliasAny;
}>();
// 事件
...
const getOldRow = () => {
  ...
  if (selectionRowKeys.value.length > 0) {
    const oldRows = getDataFromKeys(selectionRowKeys.value, props.tableData, getRowKey.value);
    if (oldRows.length) {
      oldRow = oldRows[0];
    }
  }
...
const handleExpandChange = (row: AliasAny, expandedRows: AliasAny[]) => {
  ...
  const keys = rows.map((item: AliasAny) => getRealRowKey(getRowKey.value, item));

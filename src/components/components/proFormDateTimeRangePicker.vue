...
type FormDateTimeRangePickerProps = PropFormFieldProps &
  Omit<ProFieldPropsType<'datePicker'>, 'valueType'>
const props = withDefaults(
  defineProps<FormDateTimeRangePickerProps>(),
  getProFormFieldInitProps(),
) as FormDateTimeRangePickerProps

const ProFormFieldProps: Ref<PropFormFieldProps> = computed(() => {
  return getProFormFieldProps<'datePicker'>(props, {
    type: 'datetimerange',
  })
})

const ProFieldProps: Ref<ProFieldPropsType> = computed(() => {
  return getProFieldProps(props, 'datePicker')
})

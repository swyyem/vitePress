...
export default defineComponent({
  ...
  setup(props, { slots, expose, emit }) {
    ...
    const defaultValue = getDefaultValueByMultiple(
      props.modelValue,
      props.fieldProps.multiple
    );
    ...
    const isIndeterminate = ref(
      checkStatus.value
        ? (defaultValue as []).length > 0 && !isAll.value
        : false
    );
      ...
      ([val, newOptions]) => {
        if (props.fieldProps.multiple) {
          ...
          if (realVal.length === 0) {
            ...
            realVal.length === filterDisabledOptions(newOptions).length
          ) {
            isAll.value = true;
            isIndeterminate.value = false;
          } else {
    ...
    return () => {
      ...
      const otherFieldProps = omit(props.fieldProps, [
        "checkAll",
        "checkAllText",
      ]);

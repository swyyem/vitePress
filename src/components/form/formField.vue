...
<template>
  ...
  >
    ...
    <template v-if="isLayout && !props.tooltip" #label>
      <span v-if="fieldModel?.getState('required')" class="profield-tip"
        >*</span
      >
      {{ labelText }}
    </template>
...
const realName = parentListName
  ? `${parentListName}${currentListIndex}.${originName}`
  : originName;
// 添加 prop 的计算属性
...
const hasBlur = props.effects
  ? props.effects.some((effect) => effect.trigger === "blur")
  : false;
...
onMounted(() => {
  if (!isLayout && fieldComponentRef.value) {
    const componentInstance =
      fieldComponentRef.value.childRef || fieldComponentRef.value;
    // 添加实例

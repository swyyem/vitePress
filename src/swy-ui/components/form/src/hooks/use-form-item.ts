...
import { useId } from '@swy-ui/hooks/use-id'
...
export const useFormItemInputId = (
  ...
  const inLabel = () => {
    ...
    while (parent) {
      if (parent.type.name === 'SwyFormItem') {
        return false
      }
      if (parent.type.name === 'SwyLabelWrap') {
        return true
      }
  ...
  // Generate id for SwyFormItem label if not provided as prop
  onMounted(() => {
    idUnwatch = watch(
  ...
  onUnmounted(() => {
    ...
    if (formItemContext?.removeInputId && inputId.value) {
      formItemContext.removeInputId(inputId.value)
    }
  })
